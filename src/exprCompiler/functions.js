...
export default function execFunc(name, myArgs, sheet) {
    ...
    for (var argI = 0; argI < myArgs.length; argI++) {
        let argTmp = myArgs[argI].run(sheet);
    ...
    switch (name) {
        ...
        case 'concatenate':
            return args.reduce((a, b) => a.toString() + b.toString());
        case 'count':
            return args.filter(isNotNaNAsFloat).length;
        case 'counta':
            return args.filter(x => x !== '' && x !== null && x !== undefined).length;
        case 'countblank':
            return args.filter(x => x === '').length;
        case 'countif':
            return args.filter(x => x == args[1]).length;
        case 'degrees':
        ...
        case 'fixed':
            return (args[2] ? x => x : commas)(args[0]) + (args[1] ? '.' + parseFloat(args[0]).toFixed(args[1]).split('.')[1] : '');
        case 'frequency':
            return args.slice(0, -1).filter(x => x <= args[args.length - 1]).length;
        case 'if':
        ...
        case 'max':
            return Math.max(...args.filter(isNotNaNAsFloat));
        case 'maxa':
            return Math.max(...args.map(parseNumAlways));
        case 'median':
        ...
        case 'min':
            return Math.min(...args.filter(isNotNaNAsFloat));
        case 'mina':
            return Math.min(...args.map(parseNumAlways));
        case 'mod':
        ...
        case 'product':
            return args.reduce((a, b) => a * b);
        case 'proper':
            return args[0].toString().split(/\s/).map(s => s[0].toUpperCase() + s.substr(1));
        case 'quotient':
        ...
        case 'stdev':
            ...
            return Math.sqrt(tmp.map(x => Math.pow(x - tmp2, 2)).reduce(add) / (tmp.length - 1));
        case 'stdeva':
        ...
        case 'stdevp':
            tmp = args.filter(isNotNaNAsFloat).map(parseNumAlways);
            tmp2 = tmp.reduce(add) / tmp.length;
            return Math.sqrt(tmp.map(x => Math.pow(x - tmp2, 2)).reduce(add) / tmp.length);
        case 'stdevpa':
            tmp = args.map(parseNumAlways);
            tmp2 = tmp.reduce(add) / tmp.length;
            return Math.sqrt(tmp.map(x => Math.pow(x - tmp2, 2)).reduce(add) / tmp.length);
        ...
        case 'var':
            tmp = args.filter(isNotNaNAsFloat).map(parseNumAlways);
            tmp2 = tmp.reduce(add) / tmp.length;
            return tmp.map(x => Math.pow(x - tmp2, 2)).reduce(add) / (tmp.length - 1);
        case 'vara':
            tmp = args.map(parseNumAlways);
            tmp2 = tmp.reduce(add) / tmp.length;
            return tmp.map(x => Math.pow(x - tmp2, 2)).reduce(add) / (tmp.length - 1);
...
var factorialCache = [];
function factorial(n, modifier = 1) {
